...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define "todolist.local", primary: true do |app|
    app.vm.hostname = "todolist"

    app.vm.network "private_network", ip: "192.168.33.15"
  end

  config.vm.provider "virtualbox" do |vb|
    vb.customize ["modifyvm", :id, "--name", "TodoList", "--memory", "1024"]
  end
  ...
  #config.vm.synced_folder "../../../todolist", "/webapps/todolist/todolist"
